  ...
  fun addImportedLlmModel(info: ImportedModel) {
    ...
    val setOfTasks =
      mutableSetOf(
        ...
        BuiltInTaskId.LLM_MOBILE_ACTIONS,
        BuiltInTaskId.LLM_AGENT_CHAT,
        BuiltInTaskId.LLM_SMART_SURVEILLANCE,
      )
    for (task in getTasksByIds(ids = setOfTasks)) {
      ...
      if (
        (task.id == BuiltInTaskId.LLM_ASK_IMAGE && model.llmSupportImage) ||
          (task.id == BuiltInTaskId.LLM_SMART_SURVEILLANCE && model.llmSupportImage) ||
          ...
          (task.id != BuiltInTaskId.LLM_ASK_IMAGE &&
            task.id != BuiltInTaskId.LLM_SMART_SURVEILLANCE &&
      ...
      try {
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          for (taskType in allowedModel.taskTypes) {
            ...
            if (taskType == BuiltInTaskId.LLM_ASK_IMAGE && model.llmSupportImage) {
              curTasks.find { it.id == BuiltInTaskId.LLM_SMART_SURVEILLANCE }?.models?.add(model)
            }
          }
        }
