...
import com.google.ai.edge.gallery.customtasks.smartsurveillance.SurveillancePromptGuidance
import com.google.ai.edge.gallery.customtasks.smartsurveillance.SurveillanceRuntimeSettings
...
interface DataStoreRepository {
  ...
  fun saveSurveillancePromptGuidance(guidance: SurveillancePromptGuidance)

  fun readSurveillancePromptGuidance(): SurveillancePromptGuidance

  fun saveSurveillanceRuntimeSettings(settings: SurveillanceRuntimeSettings)

  fun readSurveillanceRuntimeSettings(): SurveillanceRuntimeSettings
}
...
class DefaultDataStoreRepository(
  ...
  override fun saveSurveillancePromptGuidance(guidance: SurveillancePromptGuidance) {
    val sanitized =
      SurveillancePromptGuidance.sanitize(
        rule = guidance.rule,
        ask = guidance.ask,
    ...
    runBlocking {
      dataStore.updateData { settings ->
        settings
          .toBuilder()
          .setSurveillanceRulePromptGuidance(sanitized.rule)
          .setSurveillanceAskPromptGuidance(sanitized.ask)
          .setSurveillanceAnalysisPromptGuidance(sanitized.analysis)
          .build()
      }
    }
  }

  override fun readSurveillancePromptGuidance(): SurveillancePromptGuidance {
    return runBlocking {
      val settings = dataStore.data.first()
      val defaults = SurveillancePromptGuidance.defaults()
      SurveillancePromptGuidance.sanitize(
        rule = settings.surveillanceRulePromptGuidance.ifBlank { defaults.rule },
        ask = settings.surveillanceAskPromptGuidance.ifBlank { defaults.ask },
        analysis = settings.surveillanceAnalysisPromptGuidance.ifBlank { defaults.analysis },
      )
    }
  }

  override fun saveSurveillanceRuntimeSettings(settings: SurveillanceRuntimeSettings) {
    val sanitized =
  ...
  override fun readSurveillanceRuntimeSettings(): SurveillanceRuntimeSettings {
    return runBlocking {
      val settings = dataStore.data.first()
      val defaults = SurveillanceRuntimeSettings.defaults()
      val migratedLookbackSeconds =
        if (settings.surveillanceLookbackSeconds > 0) {
          settings.surveillanceLookbackSeconds
        } else if (settings.surveillanceAnalysisIntervalSeconds > 0) {
          settings.surveillanceAnalysisIntervalSeconds
        } else if (settings.surveillanceAnalysisFps > 0f) {
          (1f / settings.surveillanceAnalysisFps).roundToInt()
        } else {
          defaults.lookbackSeconds
        }
      SurveillanceRuntimeSettings.sanitize(
